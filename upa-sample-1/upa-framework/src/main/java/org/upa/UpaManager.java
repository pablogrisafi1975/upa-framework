...
@Singleton
public class UpaManager {
	private static Logger logger = LoggerFactory.getLogger(UpaManager.class);
			...
			for (Parameter parameter : method.getParameters()) {
				...
				if (HttpServletRequest.class.isAssignableFrom(parameterType)) {
					...
					try {
						...
						String fileName = UpaUtils.getFileName(filePart);
						args[i] = new UpaUploadFile(fileName, UpaUtils.readFully(filePart.getInputStream()));
		...
		try {
			for (Part part : httpServletRequest.getParts()) {
				if (part.getName().equals(parameterName)) {
					String fileName = UpaUtils.getFileName(part);
					UpaUploadFile upaUploadFile = new UpaUploadFile(fileName, UpaUtils.readFully(part.getInputStream()));
